...
from kumc_agent.infra.legacy.indexing.chunks import Chunk, chunk_embedding_text, load_chunks_from_dirs, write_chunks
from kumc_agent.infra.legacy.indexing.llm_client import generate_text
from kumc_agent.infra.legacy.indexing.token_utils import estimate_tokens
from kumc_agent.infra.legacy.indexing.utils import ensure_dir
...
def _embed_chunks(chunks: list[Chunk], *, config: AppConfig) -> np.ndarray:
    ...
    if not chunks:
        return np.array([])
    from kumc_agent.infra.legacy.config import EmbeddingFactory
