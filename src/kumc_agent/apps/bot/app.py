...
def main(*, base_dir: Path | None = None) -> None:
    ...
    def _access_context(interaction: discord.Interaction) -> AccessContext:
        roles = tuple(str(getattr(role, "id", "")) for role in getattr(interaction.user, "roles", []) if getattr(role, "id", None))
    ...
    def _is_authorized(interaction: discord.Interaction) -> bool:
        allowed_users = set(context.config.security.maintenance_command_author_ids)
        if allowed_users and int(interaction.user.id) not in allowed_users:
            return False
        allowed_guilds = set(context.config.security.discord_guild_allow_list)
    ...
    async def admin_action(
        ...
        if action.value == "readiness":
            ...
            payload = context.feature_flags.modes()
            summary = "feature flags"
            filename = "kumc-agent-feature-flags.json"
        elif action.value == "permissions":
            payload = {
                "maintenance_command_author_ids": context.config.security.maintenance_command_author_ids,
                "discord_guild_allow_list": context.config.security.discord_guild_allow_list,
                "admin_configured": bool(context.config.security.maintenance_command_author_ids),
                "guild_allow_list_configured": bool(context.config.security.discord_guild_allow_list),
            }
            summary = "permissions"
    ...
    @app_commands.choices(
        ...
        type=[app_commands.Choice(name="task", value="task")],
    )
    async def approval(
        interaction: discord.Interaction,
        action: app_commands.Choice[str],
    ...
    async def on_ready() -> None:
        allowed_guilds = context.config.security.discord_guild_allow_list
        if allowed_guilds:
            for guild_id in allowed_guilds:
                await bot.tree.sync(guild=discord.Object(id=int(guild_id)))
        else:
            await bot.tree.sync()
        logger.info("Wave 3 bot app ready. user=%s", bot.user)
