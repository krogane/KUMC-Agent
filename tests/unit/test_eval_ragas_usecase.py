...
class EvaluateRagasUsecaseTests(unittest.TestCase):
    def _write_eval_jsonl(self, base: Path) -> Path:
        path = base / "ragas.jsonl"
        path.write_text(
            json.dumps(
                {
                    "question": "KUMCは何のサークル？",
    ...
    def test_execute_runs_enabled_ragas_metrics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                os.environ,
                {
                    "EVAL_ANSWER_RELEVANCY_ENABLED": "1",
                    "EVAL_FAITHFULNESS_ENABLED": "0",
                    "EVAL_CONTEXT_PRECISION_ENABLED": "1",
                    "EVAL_CONTEXT_RECALL_ENABLED": "0",
                },
                clear=False,
            ):
                with patch.dict(
        ...
        self.assertEqual(fake_chat.queries, ["KUMCは何のサークル？"])
