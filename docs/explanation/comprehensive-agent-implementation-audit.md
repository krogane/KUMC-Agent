...
一方で、Planner / Verifier / 承認待ち候補作成 / payload の安定契約 / Discord承認UI / 評価・監視は部分実装に留まる。現状は「初期実装を超えた骨格実装」ではあるが、仕様上の完全実装とは言えない。
...
| PLANで入力分解、必要機能、tool順序、検索条件、成功条件、副作用境界を決定 | キーワードベースでfeatureからtoolを選ぶ。分解は「toolごとのtask」程度で、検索条件・成功条件は汎用文言に留まる | 部分実装 |
...
| VERIFYで根拠不足、矛盾、権限外情報、副作用境界違反を検出 | citation有無とcandidate statusの一部のみ確認。ACL再検証、tool間矛盾、secret混入、正本変更の前後比較は未実装 | 未達 |
...
### 1. Plannerが仕様上の「計画」に届いていない

現行Plannerは `detect_required_features()` とキーワード分岐でtool列を作る。入力を小タスクに分解しているように見えるが、実態は「各toolを実行する」というtaskを生成しているだけである。
...
- JSON schema validationを伴うplanner出力
...
- Level 4: tool別評価、監視、LLM planner/schema validation、実運用runbook
...
### 3. Planner schemaをtool別に具体化する

`AgentPlan` のschemaは存在するが、tool入力の仕様が粗い。tool別に以下を定義したほうがよい。
...
### 4. Verifierを仕様として詳細化する

Verifierは現行仕様よりさらに実装可能な形に落とすべきである。
...
3. Plannerのtool別input schemaとclarification条件を実装する。
4. Verifierにrepository前後比較、secret/context検査、tool別citation検査を追加する。
